        ...
        public async Task<IEnumerable<RealEstate>> Get(int limit, int offset, string sort)
        {
            ...
            if (_IsTable(tables, sort))
            {
                if (limit != 0 && offset != 0 && isTable)
                {
                    if (isDesc)
                    {
                        return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " DESC LIMIT @limit OFFSET @offset", parameters);
                    }
                    return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " LIMIT @limit OFFSET @offset", parameters);
                }
                else if (limit != 0 && offset == 0 && isTable)
                {
                    if (isDesc)
                    {
                        return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " DESC LIMIT @limit", parameters);
                    }
                    return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " LIMIT @limit", parameters);
                }
                else if (offset != 0 && limit == 0)
                {
                    if (isDesc)
                    {
                        return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " DESC LIMIT -1 OFFSET @offset", parameters);
                    }
                    return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " LIMIT -1 OFFSET @offset", parameters);
                }
                else
                {
                    if (isDesc)
                    {
                        return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " DESC LIMIT 10");
                    }
                    return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates ORDER BY " + sort + " LIMIT 10");
                }
            }
            else
            {
                return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates LIMIT 10");
            }
