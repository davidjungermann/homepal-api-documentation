    ...
    public class RealEstatesRepository : IRealEstatesRepository
    {
        ...
        public async Task<IEnumerable<RealEstate>> Get(int limit, int offset)
        {
            ...
            if (limit != 0 && offset != 0)
            {
                ...
                return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates LIMIT @limit OFFSET @offset", parameters);
            }
            else if (limit != 0 && offset == 0)
            {
                parameters.Add("@limit", limit, DbType.Int32, ParameterDirection.Input);
                return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates LIMIT @limit", parameters);
            }
            else if (offset != 0 && limit == 0)
            {
                parameters.Add("@offset", offset, DbType.Int32, ParameterDirection.Input);
                return await connection.QueryAsync<RealEstate>("SELECT * FROM RealEstates LIMIT -1 OFFSET @offset", parameters);
            }
            else
            {
