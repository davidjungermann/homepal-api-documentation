...
export const DeleteBuilding = ({ history, token }) => {
  return (
    <Section id="delete-building" history={history}>
      <Description>
        <TextUnit header="Delete Building" isSubsection>
          <p>
        ...
        <Table
          ...
          values={[
            {
              col1: (
                <TableItem description="The ID of the requested Building.">
                  id
                </TableItem>
              ),
              col2: "Required",
      ...
      <Usage>
        <EndpointExample header="Endpoint" copy baseUrl="api/buildings/{id}">
          <Endpoint request="DELETE" baseUrl="api/buildings/{id}"></Endpoint>
        </EndpointExample>
        <Example header="USAGE EXAMPLE" token={token} copy>
          curl --request DELETE https://localhost:6001/api/buildings/1
        </Example>
        ...
        <ContainerBlock header="RESPONSE BODY EXAMPLE" inactiveContainerBlock>
          <p>{"// "}here is no response body for this endpoint.</p>
        </ContainerBlock>
      </Usage>
    </Section>
  );
