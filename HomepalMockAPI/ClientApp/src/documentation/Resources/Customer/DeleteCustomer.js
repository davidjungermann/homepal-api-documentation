...
export const DeleteCustomer = ({ history, token }) => {
  return (
    <Section id="delete-customer" history={history}>
      <Description>
        <TextUnit header="Delete Customer" isSubsection>
          <p>
        ...
        <Table
          ...
          values={[
            {
              col1: (
                <TableItem description="The ID of the requested Customer.">
                  id
                </TableItem>
              ),
              col2: "Required",
      ...
      <Usage>
        <EndpointExample header="Endpoint" copy baseUrl="api/customers/{id}">
          <Endpoint request="DELETE" baseUrl="api/customers/{id}"></Endpoint>
        </EndpointExample>
        <Example header="USAGE EXAMPLE" token={token} copy>
          curl --request DELETE https://localhost:6001/api/customers/1
        </Example>
