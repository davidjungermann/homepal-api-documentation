...
export const GetCustomers = ({ history, token, setCurrentSection }) => {
  return (
    <Section
      id="get-customers"
      history={history}
    ...
    >
      ...
      <Usage>
        <EndpointExample header="Endpoint" copy baseUrl="api/customers">
          <Endpoint request="GET" baseUrl="api/customers"></Endpoint>
        </EndpointExample>
        <Example header="USAGE EXAMPLE" token={token} copy>
          curl --request GET
          https://localhost:6001/api/customers?limit=3&offset=1&sort=id
        </Example>
